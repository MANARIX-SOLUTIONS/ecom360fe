...
const navConfig = [
  { key: '/dashboard', permission: 'dashboard' as const, icon: <LayoutDashboard size={20} />, label: 'Dashboard' },
  { key: '/pos', permission: 'pos' as const, icon: <ShoppingCart size={20} />, label: 'POS' },
  { key: '/products', permission: 'products' as const, icon: <Package size={20} />, label: 'Produits' },
  { key: '/reports', permission: 'reports' as const, icon: <FileText size={20} />, label: 'Rapports' },
  { key: '/clients', permission: 'clients' as const, icon: <Users size={20} />, label: 'Clients' },
  { key: '/suppliers', permission: 'suppliers' as const, icon: <Truck size={20} />, label: 'Fournisseurs' },
  { key: '/expenses', permission: 'expenses' as const, icon: <Receipt size={20} />, label: 'Dépenses' },
  { key: '/settings', permission: 'settings' as const, icon: <Settings size={20} />, label: 'Paramètres' },
]
...
export default function MainLayout() {
  ...
  const navItems = useMemo(() => {
    ...
    if (isSuperAdmin) {
      items.push({ key: '/backoffice', icon: <Shield size={20} />, label: t.backoffice.title })
    }
    return items
  ...
  return (
    <Layout className={styles.root}>
      ...
      <nav className={styles.bottomNav}>
        ...
        <button
          type="button"
          className={location.pathname === '/more' || location.pathname === '/backoffice' || ['/settings', '/clients', '/expenses', '/suppliers'].some((p) => location.pathname.startsWith(p)) ? styles.navActive : ''}
          onClick={() => navigate('/more')}
          aria-label="Plus"
        >
          <MoreHorizontal size={22} />
          <span>Plus</span>
          {(location.pathname === '/more' || ['/settings', '/clients', '/expenses', '/suppliers'].some((p) => location.pathname.startsWith(p))) && <span className={styles.navDot} />}
        </button>
      </nav>
    </Layout>
  )
