...
export default function SettingsStores() {
  ...
  const { stores, activeStore, setActiveStoreId, addStore, updateStore, removeStore, hasStores } = useStore()
        ...
        {!hasStores ? (
          <div className={styles.emptyHero}>
            ...
            <Typography.Text type="secondary" className={styles.emptySubtitle}>
              Créez votre premier point de vente pour commencer à utiliser 360 PME. 
              Gérez vos stocks, enregistrez des ventes et suivez votre activité.
            </Typography.Text>
            {!storesAtLimit && (
              <Button type="primary" size="large" icon={<Plus size={18} />} onClick={openAdd} style={{ marginTop: 20, height: 48 }}>
            ...
            {stores.map((store) => (
              <li key={store.id} className={styles.storeRow}>
                ...
                <div className={styles.storeActions}>
                  ...
                  <Button type="text" size="small" icon={<Pencil size={14} />} onClick={() => openEdit(store.id)} aria-label={t.common.edit} />
                  <Button type="text" danger size="small" icon={<Trash2 size={14} />} onClick={() => handleRemove(store.id)} aria-label={t.common.delete} />
                </div>
              </li>
            ))}
          </ul>
