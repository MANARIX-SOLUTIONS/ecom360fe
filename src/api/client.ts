...
export function setAuth(tokens: { accessToken: string; refreshToken: string }, user?: { fullName: string; email: string; businessId: string; role: string; planSlug?: string }) {
  setTokens(tokens.accessToken, tokens.refreshToken)
  localStorage.setItem('ecom360_auth', 'true')
  if (user) {
    localStorage.setItem('ecom360_user', JSON.stringify({ name: user.fullName, email: user.email }))
