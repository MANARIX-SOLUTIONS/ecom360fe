...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const addStore = useCallback(async (store: Omit<Store, 'id'>): Promise<Store> => {
    if (!hasAuth) {
  ...
  const updateStore = useCallback(async (id: string, updates: Partial<Pick<Store, 'name' | 'address'>>) => {
    if (!hasAuth) {
